...
func StartExporter(d time.Duration, r metrics.Registry, sd *stackdriver.Exporter) *time.Ticker {
	...
	go func() {
		for range t.C {
			if err := publish(sd, r); err != nil {
				log.Warn("Failed to publish stackdriver metrics: %v", err)
			}
		}
	}()
	return t
...
// publish collects data from each metric in the registry and publishes it to sd.
func publish(sd *stackdriver.Exporter, r metrics.Registry) error {
	...
	return sd.ExportMetrics(context.Background(), m)
}
